...
public class PlayerMovement : MonoBehaviour
{
    ...
    [SerializeField] private AbilityUIManager dashIconManager;
    [SerializeField] private AbilityUIManager stimIconManager;
    ...
    public PlayerAudioManager GetPlayerAudioManagerScript() { return PAM_Script; }
    #endregion
    ...
    private void MovementStim()
    {
        ...
        if (Input.GetKeyDown(stimKey) && canStim)
        {
            ...
            PAM_Script.PlayStimActivateSound();

            StartCoroutine(StimCountdownTick((int)stimCooldown));
            
            Invoke(nameof(StimReset), stimDuration);
            Invoke(nameof(StimTimerReset), stimCooldown);
        }

        if (isStimmed && moveDirection.magnitude > Mathf.Epsilon)
        {
    ...
    private IEnumerator StimCountdownTick(int curTime)
    {
        stimIconManager.CooldownDisplay(canStim, curTime);

        yield return new WaitForSeconds(1f);
        curTime--;
        if (curTime > 0)
            StartCoroutine(StimCountdownTick(curTime));
    }
    ...
    private IEnumerator DashReset(float waitTime)
    {
        ...
        StartCoroutine(DashCountdownTick((int)dashCooldown));
        //dashIconManager.CooldownDisplay(canDash, (int)(dashCooldown));
    ...
    private IEnumerator DashCountdownTick(int curTime)
    {
        dashIconManager.CooldownDisplay(canDash, curTime);

        yield return new WaitForSeconds(1f);
        curTime--;
        if (curTime > 0)
            StartCoroutine(DashCountdownTick(curTime));
    }
    ...
    private void StimReset()
    {
        ...
        stimFovBoost = 1f;
        isStimmed = false;
        PAM_Script.PlayStimDeactivateSound();
    }
    ...
    {
        canStim = true;
        stimIconManager.CooldownDisplay(canStim, (int)stimCooldown);
    }
}